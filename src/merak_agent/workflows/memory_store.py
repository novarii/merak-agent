"""In-memory ChatKit store implementation for development and tests.

This mirrors the Store contract documented at
https://openai.github.io/chatkit-python/server/#data-store by persisting
threads and items inside the process. Attachments are intentionally
unsupported because dev-only usage should not accept uploads.
"""
...
class MemoryStore(Store[dict[str, Any]]):
    ...
    async def load_threads(
        ...
        window = threads[start : start + limit + 1]
        has_more = len(window) > limit
        window = window[:limit]
        next_after = window[-1].id if has_more and window else None
    ...
    async def save_attachment(
        ...
        raise NotImplementedError(
            ...
            "with proper authentication before enabling uploads."
        )
    ...
    async def delete_attachment(
        ...
        raise NotImplementedError(
            "MemoryStore does not delete attachments because nothing is stored."
        )
